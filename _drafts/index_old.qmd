...
This dissertation proposal contains three projects that further the goal of an algorithm-guided recovery monitoring and support system. Two projects (chapter 2 and 3) are completed and one project (chapter 4) is a proposed future direction. These projects are explained more below:   

- Chapter 2: Machine learning models for temporally precise lapse prediction in alcohol use disorder.

- Chapter 3: Lagged predictions of next week alcohol use for precision mental health support.

- Chapter 4: State-space models for idiographic risk monitoring and recovery support recommendations.
...
Using these EMA reports, we created datasets with `r formatC(nrow(labels_week), big.mark=",")`, `r formatC(nrow(labels_day), big.mark=",")`, and `r formatC(nrow(labels_hour), big.mark=",")` future prediction windows for the week, day, and hour window widths, respectively.  Each dataset contained 286 features and an outcome labeled as *lapse* or *no lapse*. These datasets were unbalanced with respect to the outcome such that lapses were observed in `r formatC(nrow(subset(labels_week, lapse == "yes")), big.mark = ",")` (`r sprintf("%1.1f", nrow(subset(labels_week, lapse == "yes"))/nrow(labels_week)*100)`%) week windows, `r formatC(nrow(subset(labels_day, lapse == "yes")), big.mark = ",")` (`r sprintf("%1.1f", nrow(subset(labels_day, lapse == "yes"))/nrow(labels_day)*100)`%) day windows, and `r formatC(nrow(subset(labels_hour, lapse == "yes")), big.mark = ",")` (`r sprintf("%1.1f", nrow(subset(labels_hour, lapse == "yes"))/nrow(labels_hour)*100)`%) hour windows.
...
We selected the best *baseline model* (previous lapse frequency feature only) configurations using auROCs from the *validation sets*. We evaluated these best baseline model configurations using *test set* performance to remove the optimization bias present in performance metrics from validation sets. The median auROC across the 30 test sets was moderate for the week (median=`r sprintf("%1.3f", auc_week_baseline |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.3f", auc_week_baseline |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.3f", auc_week_baseline |> pull(roc_auc) |> min())`-`r sprintf("%1.3f", auc_week_baseline |> pull(roc_auc) |> max())`), day (median=`r sprintf("%1.3f", auc_day_baseline |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.3f", auc_day_baseline |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.3f", auc_day_baseline |> pull(roc_auc) |> min())`-`r sprintf("%1.3f", auc_day_baseline |> pull(roc_auc) |> max())`), and hour (median=`r sprintf("%1.3f", auc_hour_baseline |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.3f", auc_hour_baseline |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.3f", auc_hour_baseline |> pull(roc_auc) |> min())`-`r sprintf("%1.3f", auc_hour_baseline |> pull(roc_auc) |> max())`) prediction windows.

We used the 30 test set auROCs to estimate the posterior probability distribution for the auROC of these baseline models.  The median auROCs from these posterior distributions were `r sprintf("%1.3f", ci_baseline |> filter(model=="week_baseline") |> pull(median) |> as.numeric())` (week),  `r sprintf("%1.3f", ci_baseline |> filter(model=="day_baseline") |> pull(median) |> as.numeric())` (day), and `r sprintf("%1.3f", ci_baseline |> filter(model=="hour_baseline") |> pull(median) |> as.numeric())` (hour).  These values represent our best estimates for the magnitude of the auROC parameter for each model.  The 95% Bayesian CI for the auROCs for these models were relatively narrow and did not contain 0.5 (chance performance) for any window width: week [`r sprintf("%1.3f", ci_baseline |> filter(model=="week_baseline") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci_baseline |> filter(model=="week_baseline") |> pull(upper) |> as.numeric())`], day [`r sprintf("%1.3f", ci_baseline |> filter(model=="day_baseline") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci_baseline |> filter(model=="day_baseline") |> pull(upper) |> as.numeric())`], hour [`r sprintf("%1.3f", ci_baseline |> filter(model=="hour_baseline") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci_baseline |> filter(model=="hour_baseline") |> pull(upper) |> as.numeric())`].

#### auROCs for Full Models
We next selected the best *full model* (which included all features) configurations using auROCs from the *validation sets*. We evaluated these best full model configurations using *test set* performance. The median auROC across the 30 test sets was high for the week (median=`r sprintf("%1.3f", auc_week |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.3f", auc_week |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.3f", auc_week |> pull(roc_auc) |> min())`-`r sprintf("%1.3f", auc_week |> pull(roc_auc) |> max())`), day (median=`r sprintf("%1.3f", auc_day |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.2f", auc_day |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.3f", auc_day |> pull(roc_auc) |> min())`-`r sprintf("%1.3f", auc_day |> pull(roc_auc) |> max())`), and hour (median=`r sprintf("%1.3f", auc_hour |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.3f", auc_hour |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.3f", auc_hour |> pull(roc_auc) |> min())`-`r sprintf("%1.3f", auc_hour |> pull(roc_auc) |> max())`) prediction windows. @fig-1-ema (left panel) displays the ROC curves by prediction window derived by aggregating predicted lapse probabilities across all test sets. 

The right panel of @fig-1-ema displays posterior probability distributions for the auROC for the full models by prediction window.  The median auROCs from these posterior distributions were `r sprintf("%1.3f", ci |> filter(model=="Week") |> pull(median) |> as.numeric())` (week),  `r sprintf("%1.3f", ci |> filter(model=="Day") |> pull(median) |> as.numeric())` (day), and `r sprintf("%1.3f", ci |> filter(model=="Hour") |> pull(median) |> as.numeric())` (hour).  These values represent our best estimates for the magnitude of the auROC parameter for each model.  The 95% Bayesian CI for the auROCs for these models were relatively narrow and did not contain 0.5 (chance performance) for any window width: week [`r sprintf("%1.3f", ci |> filter(model=="Week") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci |> filter(model=="Week") |> pull(upper) |> as.numeric())`], day [`r sprintf("%1.3f", ci |> filter(model=="Day") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci |> filter(model=="Day") |> pull(upper) |> as.numeric())`], hour [`r sprintf("%1.3f", ci |> filter(model=="Hour") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci |> filter(model=="Hour") |> pull(upper) |> as.numeric())`].

{{< embed notebooks/figures.qmd#fig-1-ema >}}

#### Bayesian Comparisons of Baseline vs. Full Models
We used the posterior probability distributions for the auROCs to formally compare the baseline vs. full models (matched for prediction window). The median increase in auROC for the full vs. baseline week model was `r sprintf("%1.3f", ci_diffs_baseline |> filter(contrast == "week vs. week_baseline") |> pull(median) |> as.numeric())` (95% CI=[`r sprintf("%1.3f", ci_diffs_baseline |> filter(contrast == "week vs. week_baseline") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci_diffs_baseline |> filter(contrast == "week vs. week_baseline") |> pull(upper) |> as.numeric())`], yielding a probability of `r sprintf("%1.3f", pp_diffs_baseline |> filter(contrast=="week vs. week_baseline") |> pull(difference) |> (\(x) mean(x>0))())` that the full week model had superior performance.  The median increase in auROC for the full vs. baseline day model was `r sprintf("%1.3f", ci_diffs_baseline |> filter(contrast == "day vs. day_baseline") |> pull(median) |> as.numeric())` (95% CI=[`r sprintf("%1.3f", ci_diffs_baseline |> filter(contrast == "day vs. day_baseline") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci_diffs_baseline |> filter(contrast == "day vs. day_baseline") |> pull(upper) |> as.numeric())`], yielding a probability of `r sprintf("%1.3f", pp_diffs_baseline |> filter(contrast=="day vs. day_baseline") |> pull(difference) |> (\(x) mean(x>0))())` that the full day model had superior performance.  The median increase in auROC for the full vs. baseline hour model was `r sprintf("%1.3f", ci_diffs_baseline |> filter(contrast == "hour vs. hour_baseline") |> pull(median) |> as.numeric())` (95% CI=[`r sprintf("%1.3f", ci_diffs_baseline |> filter(contrast == "hour vs. hour_baseline") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci_diffs_baseline |> filter(contrast == "hour vs. hour_baseline") |> pull(upper) |> as.numeric())`], yielding a probability of `r sprintf("%1.3f", pp_diffs_baseline |> filter(contrast=="hour vs. hour_baseline") |> pull(difference) |> (\(x) mean(x>0))())` that the full hour model had superior performance. 

#### Bayesian Comparisons of Full Models by Prediction Window
We also used the posterior probability distributions for the auROCs for the three full models to formally compare the differences in performance by prediction window width.  The median increase in auROC for the hour vs. the day model was `r sprintf("%1.3f", ci_diffs |> filter(contrast == "hour vs. day") |> pull(median) |> as.numeric())` (95% CI=[`r sprintf("%1.3f", ci_diffs |> filter(contrast == "hour vs. day") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci_diffs |> filter(contrast == "hour vs. day") |> pull(upper) |> as.numeric())`], yielding a probability of `r sprintf("%1.3f", pp_diffs |> filter(contrast=="hour vs. day") |> pull(difference) |> (\(x) mean(x>0))())` that the hour (vs. day) model had superior performance.  The median increase in auROC for the hour vs. the week model was `r sprintf("%1.3f", ci_diffs |> filter(contrast == "hour vs. week") |> pull(median) |> as.numeric())` (95% CI=[`r sprintf("%1.3f", ci_diffs |> filter(contrast == "hour vs. week") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci_diffs |> filter(contrast == "hour vs. week") |> pull(upper) |> as.numeric())`], yielding a probability of `r sprintf("%1.3f", pp_diffs |> filter(contrast=="hour vs. week") |> pull(difference) |> (\(x) mean(x>0))())` that the hour model (vs. week) had superior performance.  The median increase in auROC for the day vs. the week model was `r sprintf("%1.3f", ci_diffs |> filter(contrast == "day vs. week") |> pull(median) |> as.numeric())` (95% CI=[`r sprintf("%1.3f", ci_diffs |> filter(contrast == "day vs. week") |> pull(lower) |> as.numeric())`-`r sprintf("%1.3f", ci_diffs |> filter(contrast == "day vs. week") |> pull(upper) |> as.numeric())`], yielding a probability of `r sprintf("%1.3f", pp_diffs |> filter(contrast=="day vs. week") |> pull(difference) |> (\(x) mean(x>0))())` that the day (vs. week) model had superior performance. 
...
PPV can be increased by increasing the decision threshold; however, increasing the decision threshold will also lower the model's sensitivity. To evaluate the trade-off between PPV (i.e., precision) and sensitivity (i.e., recall) across decision thresholds, we created Precision-Recall curves by concatenating predicted lapse probabilities across the 30 test sets  (@fig-2-ema). For example, the dotted lines in @fig-2-ema depict the sensitivities (`r sprintf("%1.3f", ppv_70 |> filter(model == "Week") |> pull(recall))`, `r sprintf("%1.3f", ppv_70 |> filter(model == "Day") |> pull(recall))`, and `r sprintf("%1.3f", ppv_70 |> filter(model == "Hour") |> pull(recall))` for week, day, and hour models, respectively) associated with decision thresholds that yield 0.700 PPV for each model.
...
@fig-consort presents a CONSORT diagram of participant retention at each study milestone. 

{{< embed notebooks/ema_supplement.qmd#fig-consort >}}   

@fig-ema-1x presents participant EMA completion percentages for at least one daily EMA. 

{{< embed notebooks/ema_supplement.qmd#fig-ema-1x >}}

@fig-preds presents histograms for the predicted probabilities of lapse for all observations in the 30 held-out test sets separately by prediction window and true outcome for the full models.

{{< embed notebooks/ema_supplement.qmd#fig-preds >}}

## Chapter 3

@fig-lag-posteriors presents the distribution of posterior probabilities for auROC by model lag.

{{< embed notebooks/lag_supplement.qmd#fig-lag-posteriors >}}

@tbl-fairness presents the median difference in auROC, 95% Bayesian CI, and posterior probability that that the auROC difference was greater than 0 for all fairness comparisons separately by model.

{{< embed notebooks/lag_supplement.qmd#tbl-fairness >}}