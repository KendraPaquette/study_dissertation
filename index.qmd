...
We next selected the best *full model* (which included all features) configurations using auROCs from the *validation sets*. We evaluated these best full model configurations using *test set* performance. The median auROC across the 30 test sets was high for the week (median=`r sprintf("%1.3f", auc_week |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.3f", auc_week |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.3f", auc_week |> pull(roc_auc) |> min())`-`r sprintf("%1.3f", auc_week |> pull(roc_auc) |> max())`), day (median=`r sprintf("%1.3f", auc_day |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.2f", auc_day |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.3f", auc_day |> pull(roc_auc) |> min())`-`r sprintf("%1.3f", auc_day |> pull(roc_auc) |> max())`), and hour (median=`r sprintf("%1.3f", auc_hour |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.3f", auc_hour |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.3f", auc_hour |> pull(roc_auc) |> min())`-`r sprintf("%1.3f", auc_hour |> pull(roc_auc) |> max())`) prediction windows. @fig-1-ema (left panel) displays the ROC curves by prediction window derived by aggregating predicted lapse probabilities across all test sets.  @fig-roc-auc in the Appendix presents the individual ROC curves from each test set.
...
@fig-missing presents the proportion of missing data for all features by model. 

{{< embed notebooks/ema_supplement.qmd#fig-missing >}}

@fig-roc-auc presents individual ROC curves and AUCs for the 30 held-out test sets. 

{{< embed notebooks/ema_supplement.qmd#fig-roc-auc >}}
