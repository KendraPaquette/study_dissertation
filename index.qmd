...
From the Bayesian model we will obtain the posterior distribution (transformed back from logit) and Bayeisan CIs for auROCs for the three state-space models and comparative traditional machine learning model. To evaluate our models’ overall performance we will report the median posterior probability for auROC and Bayesian CIs. We will then conducted Bayesian model comparisons using our two sets of contrasts. For both model comparisons, we will determine the probability that the models’ performances differed systematically from each other and report the precise posterior probability for the difference in auROCs and the 95% Bayesian CIs.
