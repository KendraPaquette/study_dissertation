...
**1. Evaluate the performance (auROC) of personalized state-space models that predict immediate (next day) opioid lapse risk from geolocation and daily surveys.** This aim will allow us to assess how individual models that account for time-varying models perform over time (i.e., each month for up to 12 months). We will be evaluating state-space models independently in this study, however, model performance achieved in study 3 can be used as a benchmark for comparison.
